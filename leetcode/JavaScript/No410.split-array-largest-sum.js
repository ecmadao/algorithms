...
var splitArray = function(nums, m) {
  ...
  const split = (start, sum, M) => {
    ...
    let cur = 0;
    let min = Infinity;
    for (let i = start; i <= nums.length - M; i += 1) {
      cur += nums[i];
      const remain = tmp[M - 1] && tmp[M - 1][i + 1] || split(i + 1, sum - cur, M - 1);
      min = Math.min(min, Math.max(cur, remain));
    }
    tmp[M][start] = min;
    return min;
  };

  return split(0, SUM, m, 0);
};
