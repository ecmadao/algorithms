 ...
 * 2. double getBill(int[] product, int[] amount) returns the value of the bill and apply the discount if needed. Answers within 10^-5 of the actual value will be accepted as correct.
 *
 * Example 1:
 * Input:
...
var Cashier = function(n, discount, products, prices) {
  ...
  this.prices = products.reduce((map, product, i) => {
    map[product] = prices[i]
    return map
  }, {})
};
...
Cashier.prototype.getBill = function(product, amount) {
  this.count -= 1
  let total = product.reduce((num, p, i) => {
    return num + this.prices[p] * amount[i]
  }, 0)
  total = this.count ? total : total - (this.discount * total) / 100
  if (!this.count) this.count = this.n
  return total
};
