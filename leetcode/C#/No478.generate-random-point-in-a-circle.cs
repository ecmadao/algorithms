...
public class Solution {
    ...
    public double[] RandPoint() {
        while (true) {
            double x = RangeRandom(0f, Radius * 2) + (Coordinates.x - Radius);
            double y = RangeRandom(0f, Radius * 2) + (Coordinates.y - Radius);
            
            if (Math.Sqrt(Math.Pow(x - Coordinates.x, 2f) + Math.Pow(y - Coordinates.y, 2f)) <= Radius) return new double[]{ x, y };
        }
        return new double[2]{ Coordinates.x, Coordinates.y };
    }
}
