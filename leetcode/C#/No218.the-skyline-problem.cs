...
public class Solution {
    public IList<IList<int>> GetSkyline(int[][] buildings) {
        int[][] data = new int[buildings.Length * 2][];
        int i = 0;
        foreach (int[] building in buildings) {
            data[i++] = new int[]{ building[0], building[2] };
            data[i++] = new int[]{ building[1], -building[2] };
        }
        Array.Sort(data, (d1, d2) => {
            if (d1[0] == d2[0]) return d2[1] - d1[1];
            return d1[0] - d2[0];
        });
        ...
        foreach (int[] point in data) {
            ...
            if (maxHeight != last) {
                res.Add(new int[]{ point[0], maxHeight });
                last = maxHeight;
            }
        }
