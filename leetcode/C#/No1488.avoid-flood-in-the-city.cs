...
public class Solution {
    public int[] AvoidFlood(int[] rains) {
        ...
        Queue<int[]> drown = new Queue<int[]>();
        List<int> days = new List<int>();
        int[] res = new int[rains.Length];

        for (int i = 0; i < rains.Length; i += 1) {
            res[i] = -1;
            if (rains[i] == 0) {
                days.Add(i);
            } else {
                if (cache.ContainsKey(rains[i])) {
                    if (days.Count == 0) return new int[0];
                    int k = days.Count;

                    for (int j = 0; j < days.Count; j += 1) {
                        if (days[j] > cache[rains[i]]) {
                            k = j;
                            break;
                        }
                    }
                    if (k == days.Count) return new int[0];
