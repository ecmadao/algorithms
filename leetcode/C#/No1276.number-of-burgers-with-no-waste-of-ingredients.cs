...
public class Solution {
    public IList<int> NumOfBurgers(int tomatoSlices, int cheeseSlices) {
        ...
        int totalSmall = 4 * cheeseSlices - tomatoSlices;

        if (totalSmall % 2 == 1 || totalSmall < 0) return res;
        totalSmall /= 2;
        if (totalSmall > cheeseSlices) return res;
        res.Add(cheeseSlices - totalSmall);
        res.Add(totalSmall);
        return res;
    }
}