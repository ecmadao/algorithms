...
public class Solution {
    public int MaxIncreaseKeepingSkyline(int[][] grid) {
        ...
        int[] skylineRow = new int[row];
        int[] skylineCol = new int[col];

        for (int i = 0; i < row; i += 1) {
            for (int j = 0; j < col; j += 1) {
                skylineRow[i] = Math.Max(skylineRow[i], grid[i][j]);
                skylineCol[j] = Math.Max(skylineCol[j], grid[i][j]);
            }
        }
        ...
        for (int i = 0; i < row; i += 1) {
            for (int j = 0; j < col; j += 1) {
                int min = Math.Min(skylineRow[i], skylineCol[j]);
                if (min <= grid[i][j]) continue;
                res += (min - grid[i][j]);
            }
        }
        return res;
